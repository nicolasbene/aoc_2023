...
# Parcours chaque ligne du fichier input.txt
for l in open("input.txt").read().splitlines():
    # Utilise une expression régulière pour extraire l'identifiant du jeu
    g = int(re.match(r"^Game (\d+)", l).group(1))
    # Initialise un dictionnaire pour stocker le nombre de cubes de chaque couleur
    d = {k: 0 for k in lim}
    
    # Parcours chaque ensemble de cubes dans la ligne et met à jour le dictionnaire
    for v, k in map(str.split, re.findall(r"(\d+ \w+)", l)):
        d[k] = int(v) if int(v) > d[k] else d[k]
    
    # Vérifie si le jeu est possible en fonction des limites spécifiées
    p1 += g if all(map(lambda k: d[k] <= lim[k], d)) else 0
    # Multiplie les quantités de cubes de chaque couleur et ajoute au total
...
# def parse_line(line):
#     # Fonction pour extraire l'identifiant du jeu et les données sur les piles de cubes
#     parts = line.split(":")
#     game_id = int(parts[0].split()[1].strip())
#     piles_data = [pile.strip() for pile in parts[1].split(';')]
#     piles = []

#     for pile_data in piles_data:
#         cards = pile_data.split(',')
#         pile = [[int(card.split()[0]), card.split()[1].strip()] for card in cards]
#         piles.append(pile)

#     return game_id, piles

# def is_valid_game(piles):
#     # Fonction pour vérifier si un jeu est valide en fonction des conditions spécifiées
#     for pile in piles:
#         total_red = sum(card[0] for card in pile if card[1] == 'red')
#         total_green = sum(card[0] for card in pile if card[1] == 'green')
#         total_blue = sum(card[0] for card in pile if card[1] == 'blue')
...
# def possible_games(game_data):
#     # Fonction pour trouver les jeux possibles
#     valid_games = []

#     for game_id, piles in game_data:
#         if is_valid_game(piles):
...
#     game_data = [parse_line(line) for line in input_data.split('\n') if line.strip()]
#     valid_games = possible_games(game_data)
#     print("Possible games:", valid_games)
#     print("Sum of identifiers:", sum(valid_games))
